...
def get_high_achivers():
    thershold = int(input("기준 점수를 입력하세요 : "))
    high_achivers = [name for name,grade in grades.items() if grade >= thershold]
    if high_achivers:
        print(f"{thershold}점 이상인 학생: {','.join(high_achivers)}")
    else:
        print(f"{thershold}점 이상인 학생이 없습니다.")
